"""spec.py — RoPE → Learned Position Embedding ablation.

Replace rotary position encoding (RoPE) with GPT-2-style learned absolute
position embeddings (wpe). Two arms, same model / data / budget:

  * baseline — the reference GPT (RoPE)
  * no_rope  — NoRoPEGPT (trunk.py): learned wpe, no RoPE
"""
DEPTH = 6
LR_MAX = "3e-4"
SEED = 42
...
ARMS = [
    ("baseline", None),
    ("no_rope",  "projects.rope_ablation.trunk.NoRoPEGPT"),
]
